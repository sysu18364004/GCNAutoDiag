...
def clean_str(string1):
    ...
    without_punctuation = string1.translate(remove)
...
def preprocess_data(original_train_sentences, original_test_sentences, args):
    ...
    original_lab_freq = {}  # to remove rare words
    for sentence in original_train_sentences:
        temp = clean_str(sentence[0])
        ...
        for word in word_list:
            if word in original_lab_freq:
                original_lab_freq[word] += 1
            else:
                original_lab_freq[word] = 1   
    ...
    for sentence in sentences:
        ...
        lab_list_temp = sentence[1].split()
        doc_labs = []
        for word in lab_list_temp:
            if word in original_lab_freq and original_lab_freq[word] >= args.remove_limit:
                doc_labs.append(word)
                lab_list_dict[word] = 1
        tokenize_sentences.append((doc_words,doc_labs))
    word_list = list(word_list_dict.keys())
    lab_list = list(lab_list_dict.keys())
    len_list = [len(l) for l in tokenize_sentences[:len(original_train_sentences)]]
    if not args.easy_copy:
        print("Average Length:", sum(len_list)/len(len_list))
        

    return tokenize_sentences, word_list,lab_list
